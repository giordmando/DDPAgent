...
uses
  ...
  System.Generics.Collections, Shell.Interfaces, System.JSON, Vcl.ExtCtrls;
...
begin
  ...
  begin
    try
      ...
      // Log ma non propagare eccezioni nei callback
      WriteLn('[ShellSession] Exception in OnExit callback');
    end;
  end;
end;
...
var
  ...
  Line: string;
begin
  // Questo metodo dovrebbe essere chiamato con FLock già acquisito

  if (FOutputBuffer.Count = 0) and (FErrorBuffer.Count = 0) then
    Exit;

  OutputLines := TStringList.Create;
  ErrorLines := TStringList.Create;
  try
    // Copia e pulisci buffer
    OutputLines.Assign(FOutputBuffer);
    ErrorLines.Assign(FErrorBuffer);
    FOutputBuffer.Clear;
    FErrorBuffer.Clear;

    // Rilascia il lock prima di chiamare i callback per evitare deadlock
    FLock.Leave;
    try
      // Invia output
      ...
      begin
        ...
        begin
          try
            ...
            WriteLn('[ShellSession] Exception in OnOutput callback');
          end;
        end;
      end;
      ...
      begin
        ...
        begin
          try
            ...
            WriteLn('[ShellSession] Exception in OnError callback');
          end;
        end;
      end;
    finally
      FLock.Enter; // Riacquisisce il lock per mantenere lo stato consistente
    end;
  finally
    OutputLines.Free;
    ErrorLines.Free;
