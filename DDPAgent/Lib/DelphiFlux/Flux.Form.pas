...
constructor TFluxForm.Create(AOwner: TComponent);
begin
  inherited;
...
procedure TFluxForm.InitializeFluxDependencies;
begin
  FDispatcher := GetFluxDispatcher();
end;

procedure TFluxForm.RegisterToDispatcherMsg(const AMessageClass: TgrMessageClass;
  const AListenerMethod: TgrMessageListenerMethod);
begin
  FLock.Enter;
  try
