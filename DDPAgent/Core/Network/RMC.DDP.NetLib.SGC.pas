...
type
  ...
  protected
    procedure OnDisconnect(AConnection: TsgcWSConnection;
      ACode: Integer); override;
    procedure OnError(AConnection: TsgcWSConnection;
      const AError: string); override;
    procedure OnException(AConnection: TsgcWSConnection;
...
constructor TAgentDDPNetLibSGC.Create(const AURL: string;
...
begin
  FActionCreatorAgent := GetActionCreatorAgent;
end;

procedure TAgentDDPNetLibSGC.OnDisconnect(AConnection: TsgcWSConnection;
  ACode: Integer);
begin
  inherited;
...
procedure TAgentDDPNetLibSGC.OnError(AConnection: TsgcWSConnection;
  const AError: string);
begin
  inherited;
...
procedure TAgentDDPNetLibSGC.OnException(AConnection: TsgcWSConnection;
